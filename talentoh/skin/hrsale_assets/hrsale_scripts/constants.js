$(document).ready(function() {		
	...
	var xin_table_iess_type = $('#xin_table_iess_type').dataTable({
		...
		"aLengthMenu": [[5, 10, 30, 50, 100, -1], [5, 10, 30, 50, 100, "All"]],
		"ajax": {
            url : site_url+"settings/iess_type_list/",
            type : 'GET'
        },
		"fnDrawCallback": function(settings){
		$('[data-toggle="tooltip"]').tooltip();          
	...
	jQuery("#iess_type_info").submit(function(e){
	/*Form Submit*/
	e.preventDefault();
		var obj = jQuery(this), action = obj.attr('name');
		jQuery('.save').prop('disabled', true);
		...
		jQuery.ajax({
			type: "POST",
			url: e.target.action,
			data: obj.serialize()+"&is_ajax=28&data=iess_type_info&type=iess_type_info&form="+action,
			cache: false,
			success: function (JSON) {
				if (JSON.error != '') {
					toastr.error(JSON.error);
					...
					$('input[name="csrf_hrsale"]').val(JSON.csrf_hash);
					$('.icon-spinner3').hide();
					jQuery('#iess_type_info')[0].reset(); // To reset form fields
					jQuery('.save').prop('disabled', false);
				}
			}
		});
	...
	if(tk_type == 'document_type'){
		...
		var tb_name = 'xin_table_'+tk_type;
	} else if(tk_type == 'iess_type'){
		var field_add = '&is_ajax=47&data=delete_iess_type&type=delete_record&';
		var tb_name = 'xin_table_'+tk_type;;
	}
	...
	$('#edit_setting_datail').on('show.bs.modal', function (event) {
		...
		} else if(field_type == 'iess_type'){
			var field_add = '&data=ed_iess_type&type=ed_iess_type&';
		}
