...
class Settings extends MY_Controller {
	...
	// Contract Type > list
	  ...
	  public function company_type_list()
     {
      ...
      // Company type > list
     public function iess_type_list()
     {
		...
		$constant = $this->Xin_model->get_iess_list();

		$data = array();
        //jd=1&data=ed_company_type&type=ed_company_type&field_id=5
        foreach($constant->result() as $r) {
									 			        
	   
		$data[] = array('<span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_edit').'"><button type="button" class="btn icon-btn btn-xs btn-outline-info waves-effect waves-light" data-toggle="modal" data-target=".edit_setting_datail" data-field_id="'. $r->id_tipo . '" data-field_type="iess_type"><span class="fas fa-pencil-alt"></span></button></span><span data-toggle="tooltip" data-placement="top" title="'.$this->lang->line('xin_delete').'"><button type="button" class="btn icon-btn btn-xs btn-outline-danger waves-effect waves-light delete" data-toggle="modal" data-target=".delete-modal" data-record-id="'. $r->id_tipo . '" data-token_type="iess_type"><span class="far fa-trash-alt"></span></button></span>',
			$r->tipo, $r->porciento
		);
	...
	// Validate and add info in database
	public function iess_type_info() {
	
		if($this->input->post('type')=='iess_type_info') {		
		/* Define return | here result is used to return user data and error for error message */
		$Return = array('result'=>'', 'error'=>'', 'csrf_hash'=>'');
		$Return['csrf_hash'] = $this->security->get_csrf_hash();
			
		/* Server side PHP input validation */		
		if($this->input->post('tipo')==='' || $this->input->post('porciento')==='') {
        	$Return['error'] = $this->lang->line('xin_error_ctype_field');
		}
		...
		$result = $this->Xin_model->add_iess_type($data);
		if ($result == TRUE) {
			$Return['result'] = 'El tipo de porciento IESS ha sido agregado con éxito';
		} else {
			$Return['error'] = $this->lang->line('xin_error_msg');
		}
		$this->output($Return);
	...
	// delete constant record > table
	public function delete_iess_type() {
		...
		if($this->input->post('type')=='delete_record') {
			...
			$Return['csrf_hash'] = $this->security->get_csrf_hash();
			$result = $this->Xin_model->delete_iess_type_record($id);
			if(isset($id)) {
				$Return['result'] = 'El tipo de aporte IESS ha sido eliminado con éxito';
			} else {
				$Return['error'] = $this->lang->line('xin_error_msg');
			}
			$this->output($Return);
	...
	// Validate and update info in database
	public function update_iess_type() {
		...
		$result = $this->Xin_model->update_iess_type_record($data,$id);		
		
		if ($result == TRUE) {
			$Return['result'] = $this->lang->line('xin_iess_type_updated');
		} else {
			$Return['error'] = $this->lang->line('xin_error_msg');
		}
		$this->output($Return);
